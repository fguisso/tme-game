...
function loadConfig(name: string) {
  ...
  stagingArea.rx = config.stagingArea.rx;
  stagingArea.ry = config.stagingArea.ry;
  expansionPercentage.value = config.expansionPercentage;
...
function goToGame() {
  ...
  // Redirect to the game page with the config name as a parameter
  router.push(`/game?config=${encodeURIComponent(configName.value)}`);
}
